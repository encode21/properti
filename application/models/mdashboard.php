...
class Mdashboard extends CI_Model {
	...
	function get_info_dashb()	{
		$this->db->select('i.id,i.judul,i.tgl,u.nama,i.ket');
		$this->db->join('user u', 'u.id=i.user');
		$this->db->where('tgl >=', date('Y-m-d'));
		$this->db->where('tgl <', date('Y-m-d',strtotime(date('Y-m-d') . "+7 days")));
		$query = $this->db->get('infor i');
		
		return $query->result();
	}
	
	function get_files_dashb()	{
		$this->db->select('f.id,f.nama AS file,f.tgl,u.nama,f.url,f.ket');
		$this->db->join('user u', 'u.id=f.user');
		//$this->db->where('tgl >=', date('Y-m-d'));
		//$this->db->where('tgl <', date('Y-m-d',strtotime(date('Y-m-d') . "+7 days")));
